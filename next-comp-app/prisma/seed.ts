...
async function main() {
  ...
  // await prisma.scheduledLossBodyPart.deleteMany({}); // Comment out if data is static and verified
  // await prisma.rateSetting.deleteMany({}); // Comment out if data is static and verified
  ...
  const numberOfGeneralEmployers = 5;
  ...
  const generalMedicalProviders: MedicalProvider[] = [];
  const numberOfGeneralMedicalProviders = 8;
  ...
  for (let i = 0; i < NUMBER_OF_CLAIMS_FOR_TARGET_USER; i++) {
    const genderValue = faker.helpers.arrayElement(Object.values(Gender));
    ...
    const injuredWorker = await prisma.injuredWorker.create({
      data: {
        ...
        gender: genderValue,
        marital_status: faker.helpers.arrayElement(Object.values(MaritalStatus)),
        address_line1: faker.location.streetAddress(),
        city: faker.location.city(),
        state: "SC",
        zip_code: faker.location.zipCode("#####-####"),
    ...
    const claim = await prisma.claim.create({
      data: {
        ...
        mmi_date: mmiDate,
        permanent_impairment_rating: pirValue,
        ...
        if (scheduledPartForImpairment) {
            await prisma.claimImpairmentEntry.create({
                data: {
                    ...
                    percentageImpairment: new Prisma.Decimal(pirValue),
...
} // This is the closing brace for main() async function
...
main()
  ...
  .finally(async () => {
    ...
    console.log("Prisma Client disconnected. Seed script finished.");
  });