...
19. adjust current date to be in user's timezone
*/
...
const AwwCRCalculator: React.FC<AwwCRCalculatorProps> = ({ maxCompensationRates }) => {
    ...
    const handleNextStepOn2 = () => {
        if (validateStep(step)) {
            if (formData.specialCase === "no") {
                setPrevStep(step);
                setStep(3);
            }
            else if (formData.specialCase === "student") {
                setPrevStep(step);
                setStep(5);
            }
            else if (formData.specialCase === "volunteerFF") {
                setPrevStep(step);
                setStep(5);
            }
        }
    ...
    const validateStep = (step: number) => {
        ...
        if (step === 2) {
            if (formData.employedFourQuarters === "") {
                newErrors.employedFourQuarters = "You must indicate if the worker was employed for at least four quarters prior to the date of injury (excluding the quarter containing the date of injury).";
            }
        }
        if (step === 3) {
            [1, 2, 3, 4].forEach((q) => {
...
{/* Step 1 - should ask for the date of injury*/}
            {step === 1 && (
                <div>
                    <h3 className="mb-4 text-lg font-semibold flex">Input the date of injury. 
...
{/*Step 2 - should ask whether the employee was a special case (student, volunteer, etc.)  */}
            {step === 2 && (
                <div>
                    <h3 className="mb-4 text-lg font-semibold flex">Was the injured worker any of the following?
                    <QuestionMarkCircleIcon className="w-6 h-6 ml-1 text-gray-500" /></h3>
                    <div className="mt-2 flex flex-col space-y-4">
                        <label className="flex items-center">
                            <input
                                ...
                                name="specialCase"
                                value="student"
                                checked={formData.specialCase === "student"}
                                onChange={handleInputChange}
                                className="mr-2"
                                tabIndex={1}
                            />
                        ...
                        <label className="flex items-center">
                            <input
                                ...
                                name="specialCase"
                                value="volunteerFF"
                                checked={formData.specialCase === "volunteerFF"}
                                onChange={handleInputChange}
                                className="mr-2"
                                tabIndex={2}
                            />
                        ...
                        <label className="flex items-center">
                            <input
                                ...
                                name="specialCase"
                                value="no"
                                checked={formData.specialCase === "no"}
                                onChange={handleInputChange}
                                className="mr-2"
                                tabIndex={3}
                            />
                    ...
                    <button tabIndex={5} onClick={handleBackBtn} className="mt-4 mr-2 bg-gray-500 text-white p-2 rounded hover:bg-gray-400 hover:bg-gray-400">
                        Back
                    </button>
                    <button tabIndex={4} onClick={handleNextStepOn2} className="mt-4 bg-blue-600 text-white p-2 rounded focus:bg-blue-500 hover:bg-blue-500">
                        Next
                    </button>
                    <button tabIndex={6} onClick={resetForm} className="mt-4 bg-red-600 text-white p-2 rounded float-right focus:bg-red-500 hover:bg-red-500">
                        Reset Form
                    </button>
                </div>
            )}
{/*Step 3 - should ask whether the employee was employed for four quarters*/}
            {step === 3 && (
                <div>
                    <h3 className="mb-4 text-lg font-semibold flex">Was the injured worker employed for at least four 
                    ...
                    <h3 className="mb-4 text-lg italic flex">Note: The four quarters of employment cannot include employment during {getQuarterContainingDateOfInjury(formData.dateOfInjury)}. 
                    </h3>
...
{/*Step 4 - should ask for the gross pay for each quarter and should only be used if the employee was employed for 4 full quarters*/}
            {step === 4 && (
                <div>
                    <h3 className="mb-4 text-lg font-semibold flex">Enter the employee's gross pay for each quarter. 
                            <QuestionMarkCircleIcon className="w-6 h-6 ml-1 text-gray-500" /></h3>
                    {[1, 2, 3, 4].map((q, index) => (
                        <div key={q}>
...
{/*Step 5 - should ask for the gross pay for a time period and should only be used if the employee was employed for less than 4 full quarters*/}
            {step === 5 && (
                <div>
                    <h3 className="mb-4 text-lg font-semibold flex">Employee was employed less than four quarters.<QuestionMarkCircleIcon className="w-6 h-6 ml-1 text-gray-500" /></h3>
