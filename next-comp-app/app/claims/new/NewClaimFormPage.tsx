// app/claims/new/NewClaimFormPage.tsx
"use client";
...
export default function NewClaimFormPageContent() {
  ...
  const [employers, setEmployers] = useState<EmployerOption[]>([]); // State for employers
    ...
    if (sessionStatus === "authenticated" && session?.user?.profileId) {
        const fetchData = async () => {
            ...
            try {
                ...
                // Fetch Employers (assuming an endpoint /api/employers exists)
                // You might need to adjust the endpoint or logic if it's different
                const employersResponse = await fetch(`/api/employers`); // Adjust if profileId is needed in query
                if (!employersResponse.ok) {
                    let errData: ApiErrorData = { error: "Failed to parse employer error response" };
                     try {
                        const parsedError: unknown = await employersResponse.json();
                        if (typeof parsedError === 'object' && parsedError !== null && 'error' in parsedError && typeof (parsedError as ApiErrorData).error === 'string') {
                            errData = parsedError as ApiErrorData;
                        }
                    } catch {
                         errData.error = employersResponse.statusText || "Failed to fetch employers list.";
                    }
                    throw new Error(errData.error);
                }
                const employersData = await employersResponse.json() as EmployerOption[];
        ...
        toast({ title: "Profile Error", description: "User profile not found. Please complete your profile.", variant: "destructive"});
        router.push("/dashboard"); // Or a relevant page
        setPageLoading(false);
    }
      ...
      <form onSubmit={(e) => void handleSubmit(onSubmit)(e)}>
        <Card>
          ...
          <CardFooter className="flex justify-end">
            <Button type="submit" disabled={isSubmitting || injuredWorkers.length === 0 || pageLoading}>
